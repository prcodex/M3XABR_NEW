...
Embedded columnar vector database built on top of Apache Arrow + the Lance file format. m3xabr-core uses it as a single-table store keyed by document id, with a 2048-dim vector column for similarity search and metadata columns for prefilters (domain, source, published_at, has_vector).

## Where it's used in m3xabr-core

- `m3xabr_core/backends/vector_db.py` — only place where `import lancedb` appears. `LanceDBBackend` implements the `VectorDBBackend` protocol. The retriever calls `search(query_vector=..., filter_expr="domain = 'brazil' AND has_vector = 1.0", top_k=N)`.
- `m3xabr_core/cli.py` — opens the db path from CLI args.
...
The contract is the `VectorDBBackend` Protocol in `m3xabr_core/backends/vector_db.py`:
...
3. Re-index the corpus with the same vectors. The `RetrievedDoc` shape (`m3xabr_core/schemas.py`) is what downstream actors expect; your backend must populate `id`, `text`, `source`, `published_at`, `score`, `domain`.
