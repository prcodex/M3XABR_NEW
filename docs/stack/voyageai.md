...
Voyage AI's Python client for their embedding API. m3xabr-core uses `voyage-3-large` at 2048 dimensions for every vector — both index-time (corpus ingestion lives outside this repo) and query-time (the retriever embeds the user query before LanceDB search).

## Where it's used in m3xabr-core

- `m3xabr_core/backends/embeddings.py` — only place where `import voyageai` appears. Wrapped in `VoyageEmbeddings`, implementing the `EmbeddingBackend` protocol. `input_type="query"` for `embed()`, `input_type="document"` for `embed_batch()`.
- `m3xabr_core/cli.py` — reads `VOYAGE_API_KEY` and constructs `VoyageEmbeddings()` for the demo CLI.
...
|---|---|
| OpenAI `text-embedding-3-large` | Solid alternative, kept available as an extra (`m3xabr-core[openai]`). 3072 dim is overkill for our corpus size. |
| Cohere `embed-multilingual-v3` | Available as `m3xabr-core[cohere]`. Comparable quality but no Voyage-specific advantages disappeared. Kept as a swap option. |
...
The contract is the `EmbeddingBackend` Protocol in `m3xabr_core/backends/embeddings.py`:
