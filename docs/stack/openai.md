...
**Package:** `openai`  •  **Version pin:** `>=1.30.0`  •  **Extras group:** `m3xabr-core[openai]`  •  **Role:** Optional alternative LLM + embedding provider
...
- `m3xabr_core/backends/embeddings.py` — `OpenAIEmbeddings` class implements the `EmbeddingBackend` protocol. Lazy import (only runs when instantiated).
...
```python
from m3xabr_core.backends.embeddings import OpenAIEmbeddings
