...
**Package:** `mypy`  •  **Version pin:** `>=1.10`  •  **Extras group:** `m3xabr-core[dev]`  •  **Role:** Static type checker

## What it does

Type-checks `m3xabr_core/`. Configured in `pyproject.toml` `[tool.mypy]`:
...
- `CONTRIBUTING.md` — `mypy m3xabr_core/` listed as part of the pre-commit checklist.
- `m3xabr_core/py.typed` marker is shipped (see `[tool.setuptools.package-data]`) so downstream consumers get types for free.
