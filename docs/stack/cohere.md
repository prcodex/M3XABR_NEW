...
**Package:** `cohere`  •  **Version pin:** `>=5.0.0`  •  **Extras group:** `m3xabr-core[cohere]`  •  **Role:** Optional alternative embedding provider
...
No Cohere backend implementation is shipped yet — the SDK is reserved as a hook. Adding `CohereEmbeddings` in `m3xabr_core/backends/embeddings.py` (following the same shape as `OpenAIEmbeddings`) is a one-class contribution.
...
```python
from m3xabr_core.backends.embeddings import CohereEmbeddings
