...
Turns Python type hints into a Click-based CLI. The `m3xabr` console script (defined in `pyproject.toml` and implemented in `m3xabr_core/cli.py`) is a Typer app. Provides `m3xabr query "..."`, `m3xabr classify "..."`, etc.

## Where it's used in m3xabr-core

- `m3xabr_core/cli.py` — `app = typer.Typer()` plus the command definitions.
...
The CLI is a thin demo wrapper — most callers will drive `Pipeline(...)` directly from their own application code. If you want a different CLI shape, rewrite `m3xabr_core/cli.py` and update the `[project.scripts]` entry in `pyproject.toml`.
