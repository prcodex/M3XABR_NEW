...
Official Python SDK for Anthropic's Messages API. m3xabr-core uses it for every LLM call in the pipeline: one Haiku call each at the classifier, router, and evaluator stages, and one Sonnet call at the synthesizer.

## Where it's used in m3xabr-core

- `m3xabr_core/backends/llm.py` — the only place where `from anthropic import Anthropic` appears. Wrapped in `AnthropicLLM`, which implements the `LLMBackend` protocol.
- `m3xabr_core/cli.py` — reads `ANTHROPIC_API_KEY` from the environment and instantiates `AnthropicLLM` for the demo CLI.
...
|---|---|
| OpenAI (gpt-4o, o-series) | Equivalent quality but no compelling reason to switch from the model we've tuned the prompts against. Available as an extra (`m3xabr-core[openai]`) for callers who want it. |
...
The contract is the `LLMBackend` Protocol in `m3xabr_core/backends/llm.py`:
...
3. Update the model strings — Sonnet/Haiku names won't match your provider. Search for `model=` in `m3xabr_core/actors/` to find call sites.
