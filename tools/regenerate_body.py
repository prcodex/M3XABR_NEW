...
Reads:
  - m3xabr_core/actors/*.py    (actor list + first-line docstrings)
  - expertises/*.md            (inventory + frontmatter)
  - pyproject.toml             (dependency list)
  - tests/*.py                 (test counts)
  - m3xabr_core/__init__.py    (public API surface)
  - all source files           (env-var grep)

Writes:
  - BODY.md                    (only between <!-- AUTOGEN:start:NAME --> and <!-- AUTOGEN:end:NAME --> markers)
...
`--check` mode: exits 1 if BODY.md is out of sync (rewrites a temp copy and diffs).
"""
...
# ---------------------------------------------------------------------------
def gen_actors() -> str:
    actors_dir = REPO / "m3xabr_core" / "actors"
    ...
    for p in sorted(actors_dir.glob("*.py")):
        ...
        try:
            ...
            module_doc = "(unable to parse)"
        rows.append(f"| `{p.stem}` | [`m3xabr_core/actors/{p.name}`](./m3xabr_core/actors/{p.name}) | {module_doc or '_(no docstring)_'} |")
    return "\n".join(rows)
...
def gen_expertises() -> str:
    ...
    for p in sorted(exp_dir.glob("*.md")):
        ...
        rows.append(f"| {name} | [`expertises/{p.name}`](./expertises/{p.name}) | {tokens} | {triggers} |")
    return "\n".join(rows)
...
def gen_env_vars() -> str:
    matches = set()
    pat = re.compile(r"os\.(?:getenv|environ\.get|environ\[)\s*\(?\s*[\"']([A-Z][A-Z0-9_]+)[\"']")
    ...
    pat2 = re.compile(r"os\.(?:getenv|environ\.get|environ\[)\s*\(?\s*[\"'](" + "|".join(re.escape(m) for m in matches) + r")[\"']")
...
def gen_public_api() -> str:
    init = REPO / "m3xabr_core" / "__init__.py"
    if not init.exists():
        return "_(no m3xabr_core/__init__.py)_"
...
def gen_tree() -> str:
    rows: list[str] = []
    rows.append("```")
    top = sorted(p for p in REPO.iterdir() if not p.name.startswith(".") and p.name not in {"__pycache__", "node_modules", ".venv", "venv"})
    for p in top:
        if p.is_dir():
...
def gen_meta() -> str:
    ...
    return f"- **Last regenerated:** {ts}\n- **Commit:** `{sha}`\n- **Generated by:** `tools/regenerate_body.py`"


# ---------------------------------------------------------------------------
# Frontmatter parser (best-effort, no PyYAML dependency)
# ---------------------------------------------------------------------------
...
def regenerate(body_text: str) -> str:
    # First pass: regenerate ALL sections except meta. Meta has a timestamp +
    # commit SHA that would otherwise make every run "out of sync".
    ...
    # If nothing else changed, leave meta alone (avoid timestamp churn).
...
def _strip_meta(text: str) -> str:
    """Replace the meta section content with a placeholder for change detection."""
    pattern = re.compile(
        r"(<!-- AUTOGEN:start:meta -->).*?(<!-- AUTOGEN:end:meta -->)",
        re.DOTALL,
    )
